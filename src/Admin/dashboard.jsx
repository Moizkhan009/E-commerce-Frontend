...
import { addProduct, fetchProduct } from '../redux/products/products_action';
 ...
 const ProductDashboard = () => {
  ...
  // Redux state se products fetch karo
            ...
            <div>
              ...
              {status === 'succeeded' && products.length === 0 && (
                <div className="text-center py-12 bg-white rounded-lg shadow">
                  ...
                  <p className="text-gray-500 text-sm mt-2">Click "Add Product" to create your first product</p>
                </div>
              )}
              ...
              {status === 'succeeded' && products.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {products.map((product) => (
                    <div key={product._id || product.id} className="bg-white rounded-lg shadow hover:shadow-lg transition overflow-hidden">
                      {product.badge && (
                        <div className="relative">
                          <span className={`absolute top-4 left-4 ${product.badgeColor} text-white text-xs font-bold px-3 py-1 rounded-full z-10`}>
                            {product.badge}
                          </span>
                        </div>
                      )}
